...
- A single newline is a *soft* break in CommonMark and renders as a space. That rule exists so a prose file can be wrapped in its source without changing its output, and it is the wrong rule for a card: pressing Enter to put "am / is / are" on three lines has to produce three lines. `egui_commonmark` has no option for this (its parser emits `SoftBreak` and the renderer draws a space), so `ui/markdown.rs` rewrites the source into hard breaks just before rendering, leaving fenced code blocks and block boundaries alone. Two alternatives were rejected: teaching the user CommonMark's two-space rule, which is a footgun with no visible cause, and storing the two spaces in the database, which would push a rendering detail into content that `db/` and `sync/` carry around forever. What is saved stays exactly what was typed.
